import { BookPlus, ChartCandlestick, CornerDownRight, CornerUpRight, LibraryBig, Settings, UserPlus, Users } from "lucide-react";
...
export default function Sidebar({currentPath = '/dashboard'}: {currentPath: string}){
    const navigation = [
        ...
        {name: "Lend Book", href: "/lend-book", icon: <CornerUpRight />},
        {name: "Return Book", href: "/return-book", icon: <CornerDownRight />},
        {name: "Add Member", href: "/add-member", icon: <UserPlus />},
        {name: "Settings", href: "/settings", icon: <Settings />},
    ]

    return <div className="sticky left-0 top-16 bg-white box-border border-r border-stone-300 min-h-full h-[calc(100vh-4rem)] py-4 pl-4 pr-0 z-10 w-fit md:min-w-56">
        <nav className="space-y-2">
            <div className="text-sm font-semibold text-stone-600 uppercase hidden md:block">
                Quick Actions
            </div>
